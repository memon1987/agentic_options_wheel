...
- **Follow-up (2026-09-01, PR #107 `c0dc2fd`):** the 14-DTE reasoning was incomplete — the invisible killer of otherwise-qualifying strikes was the `min_open_interest: 500` gate evaluated against a hardcoded `open_interest: 0` (chain source), plus a `min_stock_price` KeyError that would have failed every candidate at the opportunity builder. Both fixed/suspended; see `fc-075-phase-2.md` §Post-execution amendment and FC-097/098/099. The `dte_too_high` collapse noted above was observed (939 GOOGL / 328 UNH per scan), so 14 DTE stays.
