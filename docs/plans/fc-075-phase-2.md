...
## Post-execution amendment (2026-09-01, PR #107 `c0dc2fd`)

- **DD-3 inventory validator — OI half SUSPENDED.** `AlpacaClient.get_options_chain` builds contracts from the snapshot endpoint and hardcodes `open_interest: 0`, so the OQ-1 starting value `min_open_interest 500` rejected **every** strike that survived the other gates (prod 2026-08-28: ~114 GOOGL / ~86 UNH per scan, invisibly — the STAGE 8 line did not emit the bucket). `config/covered_call.yaml` now sets `min_open_interest: null`; `max_spread_pct: 0.10` stays active. `tests/test_cc_stock_metrics_keys.py::TestCoveredCallOpenInterestFloorSuspended` pins the suspension to the hardcode. Restore via **FC-097** (source OI from `/v2/options/contracts`, re-derive the floor). `docs/BACKTEST_ENGINE.md` had already recorded the hardcode on 2026-08-28 for the sim allowlist — the live config was never cross-checked against it.
- **Screening keys were missing on this profile.** `get_stock_metrics` hard-indexes `strategy.min_stock_price` / `max_stock_price` / `min_avg_volume`; the CC yaml had none, so every chain fetch logged `stock_metrics_error` and `_create_call_opportunity` would have failed closed. Keys added (non-binding, wide) and their validation hoisted to every profile in `Config._validate_config`. Deferrals: FC-098 (severity mapping + CC scan-zero alert + narrow the broad except), FC-099 (put-side validation bucket).
