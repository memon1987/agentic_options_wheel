...
- **PR:** https://github.com/memon1987/agentic_options_wheel/pull/100
- **Commit:** `2bad351` (squash; branch `5ee2cd5` build, `a09ddc3` review fixes Q1–Q12, `9bc9ec7` confirmation fixes S1–S4)
- **Date:** 2026-08-28
- **Notes:** **Acceptance met by 4–7×: 10 scenarios × 6 symbols × 1 year = 17.1 s wall (30.5 s with sensitivity; 190 s as independent runs on main), 0 provider calls during replays; `ChainStore.get` 5× faster; AAPL warm pass 3.6 s → 0.9 s.** Byte-identical to main on AAPL 2026-05 and NVDA 2025-10→2026-01 (`PYTHONHASHSEED=0` — main itself is nondeterministic on tie order, FC-092). Reviews (fidelity + data/perf) confirmed the engine (9 extreme replays leave `Materialised` bit-identical; D5 identity 0 mismatches on all 5,424 files; Job peak RSS 368 MB) and required reporting/cache/test fixes: in-sample banner by default, low-activity cells (`days_in_position_fraction < 0.25`) excluded from medians and rendered `low-act N%` (this alone demoted an arm that looked best on 2–23%-deployed cells), `scenario_hash` per arm (12/20 sweepable keys sit outside `config_hash`), `base` with overrides refused, `rolling.fallback_strike_attempts` rejected (adapter fills rung 1 unconditionally), empty provider answers never frozen as bar coverage, fetches vs bar-cache hits counted separately, a conftest guard against tests leaking logging config (two leaks fixed), the real-cache identity check moved to `tools/diagnostics/chain_store_identity_check.py`. Confirmation 1 found `insufficient` cells double-counted as `low-act` → fixed (`insufficient` wins; states partition). Deviations recorded in the Status header. Follow-ups: FC-091 (merge-on-put — next), FC-092, FC-093. First intended use: FC-055 / FC-001 as sweeps.
