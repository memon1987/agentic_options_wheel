...
**Status:** Consideration (filed 2026-08-27, operator-requested) — **2026-09-17: recommendation recorded (framing 5, remaining-yield rule) + a shared-path metric defect found; ready for a plan when the operator has bandwidth**
...
5. **RECOMMENDED (2026-09-17) — replace the M/E ladder with a forward-looking remaining-yield rule.** `remaining_yield = (M / S) × 365 / max(DTE, 1)` where M = current option mark per share, S = stock price; close when `remaining_yield < hurdle` OR `M <= floor_per_share`. Rationale: (a) the entry premium E is sunk — whether to keep a short call open depends only on what it still pays vs the time and risk left; (b) it is the SAME quantity the scanner ranks entries by (annualized return), so entry and exit stop speaking different languages ("hold while it still pays like a trade you would open"); (c) time is inside the formula, so the DTE ladder — and this FC's cliff class — is deleted rather than re-tuned; (d) scale-free across symbols and premium sizes. Starting knobs: hurdle ≈ half the entry-side yield (CC entries 09-01→09-16 were written at 12–25% annualized → 8%), floor $0.10/share, plus an explicit expiry-day rule (`DTE <= 1 and M <= 0.25 → close`) instead of bands. Known limits: annualization explodes at low DTE (the floor + clamp handle the tail); the rule HOLDS through an approaching-the-strike move because rising M raises the yield — correct, that is the roller's job (`itm_trigger_ratio`), and above basis assignment is a gain; it does not fix the re-entry strike ratchet (separate guard: do not re-sell below the strike just closed for N days). Needs S at monitor time — the monitor already fetches quotes, and `should_close_call_early` receives `current_option_data` that Phase 1 left unused. Rejected alternative: exit on current delta (< ~0.05) — nearly equivalent but needs Greeks from the indicative feed and is not what the entry side optimizes. Pre-validate in the simulation studio (FC-096) against the wheel's lake before it touches either live profile.

**Metric defect found 2026-09-17 (shared path, since the initial commit `e406363`, unfiled elsewhere):** `should_close_call_early` (`src/strategy/call_seller.py:440`) computes `profit_percentage = unrealized_pl / |market_value|` — profit over the COST TO CLOSE, not over the premium collected. The yaml bands (0.35–0.80) and every comment read as "% of premium captured". The two are monotone transforms of each other (engine_t = c/(1−c)), so this is not "two metrics" — it is one statistic mislabelled, and every band fires EARLIER than configured: 35% → 25.9% captured, 50% → 33.3%, 60% → 37.5%, 80% → 44.4%. `should_close_put_early` (`src/strategy/put_seller.py:456`) has the IDENTICAL denominator, so the wheel's put exits are mislabelled the same way — both legs, both profiles. FC-015's 2026-05-12 validation ("42/42 events match new config") was read off the engine's own `profit_pct` field, so it validated the mislabelled metric against itself. Either fix belongs INSIDE this FC — relabelling alone is not worth a plan; framing 5 makes the question moot.

**Live audit, covered-call profile 2026-09-01→09-16 (`docs` of record: session memory 2026-09-16; source = `call_profit_target_reached` payloads + `covered_call.trades_from_activities`):** 8 calls written, 7 closed by the profit taker, all at the 09:55 ET monitor, 1–5 days after sale. OQ-1 below is ANSWERED: exits DO cluster at the DTE-7/35% band (#1 GOOGL C352.5 closed at DTE 7 one day after sale; #6 UNH C400 at DTE 7 two days after) and the outcome in this window WAS bad — 6 of 7 bought-back calls expired or are expiring worthless ($386 of the $486 buyback spend given back; hold-to-expiry ≈ $1,210 vs $724 realized), and the denominator defect changed one decision of seven (#7 UNH C402.5 9/25: 41.5% captured vs a 50% band; #3 marginal at 51%). Under framing 5 (hurdle 8%, floor $0.10) only #2 (UNH C402.5 9/04, mark 0.13 at DTE 0) and #4 (UNH C415, 5.8%) would have closed. The larger cost is not the buyback but the RE-ENTRY RATCHET: each close re-anchored the [0.15, 0.25] delta band to the new price (GOOGL C352.5 → C347.5 → C342.5 → C345 while the stock fell then rebounded), leaving GOOGL C345 at-the-money with no roller running. Caveats: seven trades on a two-week falling tape is illustration, not validation; stock prices at trigger were approximated from daily closes. Instrumentation ask for the plan: add `entry_price`, `captured_pct_of_premium` and `remaining_yield` to the `call_profit_target_reached` / `early_close_executed` events so the next audit needs no fill-ledger reconstruction.
...
**Open questions:**
- ~~Do the first weeks of live CC 14-DTE exits confirm the predicted ~day-7/35% exit clustering, and is that outcome actually bad?~~ **Answered 2026-09-17 (see the live audit above): yes on both counts for the first two weeks; re-check on a rising tape before treating it as settled.**
...
- ~~Dedup keyed on `git_commit`~~ **EXECUTED by FC-096 PR-a (2026-09-01):** the key is now a content hash of `src/**` + `requirements.txt` (engine identity); docs-only pushes no longer invalidate
...
### FC-106: simulation console layout — simplify what Phase E shipped

**Scope:** shared
**Status:** Filed 2026-09-04 (operator feedback after the Phase E smoke: "the current dashboard layout looks very complex and hard to follow")
**Size estimate:** M (UI-only; no engine or backend change expected)

**Problem:** Phase E delivered every signed component (verdict strip, price/equity/premium/drawdown/deployment charts, ledger + cycle tables, rejection and forecast panels, tweak bar, compare view, provenance and bias footers) on one long page. Each element is individually correct and guarded, but the whole reads as a wall: a PM cannot tell at a glance what the run says, what to look at next, or which controls matter. The guardrail text (in-sample banners, withheld notices, caveats, provenance) competes visually with the evidence it qualifies.

**Proposal (to be planned, not decided here):** a progressive-disclosure pass — verdict strip and one headline chart first, everything else behind labelled sections that open on demand; the tweak bar and compare entry as a single "what if" affordance; guardrail text moved into consistent, quieter placements (a caveat drawer, badges) without weakening it; the compare view's matrix collapsed to a one-line summary with the full table on demand. Plan must state which guardrail placements are load-bearing (the in-sample banner must stay above the fold; the bias footer must stay last) and carry the Phase E amendments forward unchanged. Measure with the operator's own walkthrough: can a new viewer answer "is this arm better than base, and on what evidence?" in under a minute.

**Also in scope (2026-09-09):** surface the covered-call scalars (`premium_yield_on_lot`, `coverage_fraction`, `rolls_executed`, roll-skip counts) on the results grid cell — today only the artifact carries them, so the grid shows `—` for a CC cell.
...
**Scope:** wheel
**Status:** EXECUTING — PR-1 `989ef78` (wheel replay roll reach 7→21, ENGINE_VERSION fc-112-wheel-roll-reach) and PR-2 `bc029a1` (frozen pre-registered read + diagnostic tool) both merged 2026-09-12. Awaiting operator steps (lake widen, pins, one-shot per PR #133's runbook); the read follows the first Saturday with pin rows. No config change until a verdict.
...
**Scope:** covered_call
**Status:** COMPLETED 2026-09-17 — PR #127 merged; operator steps O1–O9 done; O6 dry-run PASSED (GOOGL C345 → C347.5 10/02 pair, $394 credit, pricing path exercised); `cc-roll-daily` enabled 2026-09-17, first live cycle 15:30 ET. Plan `docs/plans/fc-100.md` Done.
...
**Scope:** shared
**Status:** COMPLETED 2026-09-12 — PR #131 squash-merged `c019b8d` (plan `docs/plans/fc-117.md`, Done). The battery now composes wheel (14) → covered_call (14, same universe, `strategy: covered_call`) → pins; `data-backfill` Job at 2 GiB. Operator: one manual execution with the peak-memory gate + alert-policy re-apply before the first CC Saturday (2026-09-19).
**Size estimate:** S–M
...
**Status:** COMPLETED 2026-09-09 — all five phases shipped and verified (A `fc-096-a`, B `fc-096-b`, C `fc-096-c`, D `fc-096-d`, E `fc-096-e`); follow-ups filed FC-101–FC-116; the console layout revision is FC-106.
...
**Phases (build order):**
- **A — data foundation: SHIPPED 2026-09-01** (plan `docs/plans/fc-096-a.md`, Done; all 14 live symbols at 22-reach; weekly Job + scheduler + 3 alert policies live; DTE ≤ 21 sweepable and live-verified end-to-end). Original scope line:  `candidates` universe beside `stocks.symbols`; weekly backfill Job (weekend cadence — Alpaca-quota courtesy, FC-095): incremental lake+bars for live+candidates at DTE 7/14/21 reaches; candidate onboarding path (one-time async materialise, ~5-20 min/symbol-year) — absorbs FC-060 Layer 3b.
- **B — sim service: SHIPPED 2026-09-02** (plan `docs/plans/fc-096-b.md`, Done; live-verified: `POST /simulate` 202 → ~30 s background replay on instance billing → done; dedup-hit resubmit **0.87 s**; per-cell artifacts streamed via the dashboard; engine-identity dedup survives docs/dashboard deploys; FC-092 fixed; rolling pins + Saturday battery deployed — rolling proof lands after two Saturdays). Was: IN PROGRESS (plan `docs/plans/fc-096-b.md`; PR-a engine-identity+FC-092 MERGED and live-verified; PR-b artifacts in review — **decided boundary: the detail artifact carries trades/curve/cycles/tally only; Phase E reads price series/B&H from its own bars path**). Original scope line:  slim engine-only image; private Cloud Run service (invoker = dashboard SA) wrapping the runner; persists to `scenario_runs`; dedup re-key to engine identity; pinning; 4th deploy chain in `cloudbuild.yaml`.
- **C — covered-call selector: SHIPPED + verified 2026-09-09** (PR #129 `f6a5a4d` + hotfix `d9763a9`; first live CC sim `a5c9c5626b4148eb` verified; plan Done). Decisions: re-seed at next close; rolling at 1.00 (FC-100 first). Was: `strategy` field in the spec; profile-aware replay; synthetic-lot seeding; CC gating honoured in the engine.
- **D — IAP cutover + roles: SHIPPED 2026-09-02/03** (plan `docs/plans/fc-096-d.md`, Done; PR-1 #119 `671285f` roles layer; operator console session verified live — custom OAuth client, `allUsers` removed, scheduler 200 through IAP; PR-2 #120 `5efe76d` token retired, session-expiry minimum, `claude-operator` SA as the programmatic operator; pin proof passed; residual = secret unbind-then-delete. FC-094 closed by it.)
- **E — PM console: SHIPPED + operator-verified 2026-09-04** (plan `docs/plans/fc-096-e.md`; six PRs #121–#126, each two adversarial reviews + confirmation; operator browser smoke passed 2026-09-04; operator verdict: works but the layout is too complex → FC-106; FC-101–105 filed from the reviews). Inventory as signed 2026-08-31: (1) verdict strip (returns vs base & vs buy-and-hold, premium, fees, win rate, drawdown, cycles, capital deployed-vs-idle); (2) per-symbol price chart with trade-event markers (put sold / assigned / call sold / called-away / roll, hover detail) — tabs per symbol + a portfolio-level equity view; (3) equity vs B&H vs base, cumulative + monthly premium, drawdown, collateral-utilisation charts; (4) full trade-ledger table (exportable); (5) wheel-cycle table (per-cycle P&L split, RoC, outcome); (6) rejection panel ("why didn't it trade" — candidate evaluation's other half; FC-092 caveat until fixed); (7) forecasted income as a RANGE (holdout vs fit run-rate bounds), bias footer mandatory; (8) config/provenance footer (overrides vs base, synthetic-lot label, engine version, freshness); plus a **side-by-side compare view** (two configs' timelines/curves on one screen). Data note: the engine already records all of this (`LedgerEvent`, `DailyState`, `WheelCycle`, rejection tallies, B&H) — today it is discarded after aggregation; the sim service persists a per-run **detail artifact** to GCS (~50-200 KB compressed), aggregates stay in BigQuery, console fetches detail on demand. Field-per-input controls; comparison always anchored to current base; equity/income/drawdown charts; forecasted income as premium run-rate extrapolation **with the bias footer and holdout discipline** — the FC-060 multiple-comparisons guardrails (per-symbol grids, insufficient/low-activity exclusion, sign agreement, in-sample banner) carry over wholesale; a PM-persona UI makes overclaiming easier, and the guardrails are the counterweight. **Also in Phase E (named in FC-096 Phase D PR-2, review round 1):** *IAP session refresh* — implement Google's documented `?gcp-iap-mode=DO_SESSION_REFRESH` handler rather than a bespoke iframe. Phase D deliberately ships only the minimum (`X-Requested-With` on every fetch, one banner in the app shell, polling stops) because a Google-account IAP session is tied to the Google login and expiry is therefore rare; the refresh handler is the real fix when it stops being rare.
