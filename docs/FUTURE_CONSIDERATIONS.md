...
**Scope:** wheel
**Status:** Filed 2026-09-08 (operator, after the FC-100 discussion: the covered-call profile is set to 1.00; the wheel's 0.98 is an FC-078 decision to be re-decided on numbers). **QUEUED BEHIND FC-116**, which supplies the number this study reads: before it, no roll-credit aggregate existed anywhere in the engine, the report, the artifact or the row, and the ITM/OTM split was `None` on every wheel row. FC-116 adds `roll_net_credit` / `itm_roll_credit` / `otm_roll_out_credit` (all PRE-FEE, matching the live roller's own `net_credit`), un-gates the split onto wheel rows, puts `roll_skips` and the rest on `scenario_runs` as columns, and — the point — makes a replayed roll's credit the one the live credit invariant was computed on rather than a haircut-from-mid number that flattered every OTM re-write by ~1.5 half-spreads. Start after FC-116's Rollout step 4 is recorded.
...
**Scope:** shared
**Status:** EXECUTING 2026-09-11 — built from `docs/plans/fc-117.md` rev 3 on branch `fc-117-cc-battery-set`; PR https://github.com/memon1987/agentic_options_wheel/pull/131 open, two adversarial reviews pending. (Plan APPROVED 2026-09-11 — two adversarial plan reviews, union addressed in rev 2, scoped confirmation's five text defects fixed in rev 3; FC-118/FC-119 filed from the reviews. Operator decision 2026-09-11: same mechanisms for both strategies.) **Two operator steps after merge:** the manual `data-backfill` execution at 2 GiB with peak memory recorded BEFORE the first Saturday (gate: ≤ 75 % of 2 GiB), and re-applying `battery_degraded_alert_policy.json` so its runbook names `standing:cc:<SYMBOL>`. The command sheet is in the PR body; the numbers belong in the plan's §Execution and here.
**Size estimate:** S–M
