...
DTE_REACH_BIAS = (
    ...
    "biases selection rather than merely adding noise", (
        ...
        "`roll_skips.no_suitable_replacement`, not against `puts_sold`. A "
        "wheel arm that overrides a DTE key reaches past 7 on entries too, and "
        "then the caveat applies to both."),
)
...
ROLL_REACH_BIAS = (
    ...
    "credits are biased DOWN", (
        ...
        "\n\nThe WHEEL's horizon (7 + 14 = 21) is fully materialised since "
        "FC-112, so the wheel has NO residual roll-reach truncation — 21 is "
        "both what its roller wants and what the lake stores. Its roll "
        "candidates past 7 DTE carry the thin-print caveat instead "
        "(`DTE_REACH_BIAS`); its ENTRIES are still capped at 7 by the scanner. "
        "Wheel roll counts and credits written before "
        "`fc-112-wheel-roll-reach` were measured on a 7-DTE ladder and are NOT "
