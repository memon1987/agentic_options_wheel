...
//
//   * `reconcile` recomputes SIX quantities the engine already persists on the
//     row (option P&L, max drawdown, puts, calls, completed cycles, final
//     equity) and the tests assert each equals the row's value on a REAL
//     captured artifact. It exists for the test. The strip DISPLAYS the row's
//     numbers, never `reconcile`'s — the point is to catch this module drifting
//     from the engine, not to become a second engine.
...
export function computeDigest(
  ...
  const reconcile: DigestReconcile = {
    ...
    finalEquity: artifact.daily.length ? artifact.daily[artifact.daily.length - 1].equity : 0,
  };
